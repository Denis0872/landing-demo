...
- persisted VM tuning in `/etc/sysctl.d/99-liza-alert.conf`
- created `systemd` unit `/etc/systemd/system/liza-alert.service`
- enabled `liza-alert.service` on boot
...
- `systemctl is-enabled liza-alert.service nginx`
- `systemctl is-active liza-alert.service nginx`
- `curl http://localhost/api/v1/lost-cases` on VPS
- `curl http://185.21.8.116/api/v1/lost-cases` from local environment
- reviewed `liza-alert.service` status and `nginx -t`
